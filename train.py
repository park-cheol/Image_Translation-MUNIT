...
def sample_images(batches_done):
    ...
    for img1, img2 in zip(imgs["A"], imgs["B"]):
        ...
        X2 = Variable(X1.type(Tensor))        # Get random style codes
        ...
        X21 = Dec1(c_code_2, s_code)
        # Concatenate samples horisontally
        X12 = torch.cat([x for x in X12.data.cpu()], -1)
        X21 = torch.cat([x for x in X21.data.cpu()], -1)
        img_sample = torch.cat((img1, X12, img2, X21), -1).unsqueeze(0)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    print('epoch time: ', datetime.timedelta(seconds=sec))
